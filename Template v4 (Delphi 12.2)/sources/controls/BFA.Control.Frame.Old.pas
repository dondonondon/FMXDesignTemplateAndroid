...
//    property DestroyAfterClose : Boolean read FDestroyAfterClose write FDestroyAfterClose;
    ...
    procedure NavigateTo(AAlias : String); overload;
    procedure NavigateTo(AFrame : TFrameContainer); overload;
...
//      ShowMessages('Tap twice to exit application');
  ...
  if ListRoute.Count > 1 then NavigateTo(ListRoute[ListRoute.Count - 2]) else
    NavigateTo(ListRoute[ListRoute.Count - 1]);
...
procedure TFrameCollection.NavigateTo(AFrame: TFrameContainer);
begin
  CallingFrame(GetAliasName(AFrame.ClassName));
end;

procedure TFrameCollection.NavigateTo(AAlias: String);
begin
