...
constructor TToasMessage.Create;
begin
  FListTemporaryToast := TList<TLayout>.Create;
  FListTemporaryPopup := TList<TLayout>.Create;
end;

procedure TToasMessage.defClickPopUp(Sender: TObject);
begin
...
procedure TToasMessage.defClickToast(Sender: TObject);
var
  lo : TLayout;
  loToast : TLayout;
begin
...
destructor TToasMessage.Destroy;
begin
...
procedure TToasMessage.flFinish(Sender: TObject);
var
  lo : TLayout;
  loToast : TLayout;
begin
...
procedure TToasMessage.Loading(IsState: Boolean);
var
  FLayout : TLayout;
  FAni : TAniIndicator;
begin
...
procedure TToasMessage.ShowPopUpMessage(FMessage: String; FJenis: TTypeMessage;
  FProc: TProc);
var
  lo : TLayout;
  reBlack : TRectangle;
...
begin
  ...
  if FJenis = TTypeMessage.Success then begin
    FStatus := 'Success!';
    FColor := $FF4BC961;
  end else if FJenis = TTypeMessage.Error then begin
    FStatus := 'Error...';
    FColor := $FFFF6969;
  end else if FJenis = TTypeMessage.Information then begin
    FStatus := 'Information';
    FColor := $FF36414A;
  end;
...
procedure TToasMessage.ShowToastMessage(FMessage: String; FJenis: TTypeMessage);
var
  lo : TLayout;
  loPop : TLayout;
  reBlack : TRectangle;
...
procedure TToasMessage.StartLoading;
begin
  Self.Loading(True);
end;

procedure TToasMessage.StopLoading;
begin
  Self.Loading(False);
end;
